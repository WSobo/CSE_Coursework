# assignment: programming assignment 1: Hangman
# author: William Sobolewski
# date: June 27th, 2023
# file: hangman.py is a program that allows you to play game known as hangman through the python terminal
# input: sys.stdin: length of the word you want to guess, and the number of lives you want to start with. Then you input one letter at a time to try to guess the word
# output: sys.stdout the text corresponding to the game, keeps track of your inputs and the progress of the game
...
def import_dictionary(filename):
    dictionary = {length: [] for length in range(3, 13)}
    try:
        with open(filename, 'r') as file:
            for line in file:
                word = line.strip()
                length = len(word)
                if length <= 12 and length >= 3:
    ...
    def hang_game(self):
        ...
        while True:
            ...
            print(" ".join(self.hidden_letters),
                  f" lives: {self.lives}", "X" * self.deaths + "O" * self.lives)
